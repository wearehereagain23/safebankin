...
async function updateCreditUI() {
    ...
    // Changed source to 'safe-bankin_admin' table
    const { data } = await supabase.from('safe-bankin_admin').select('history_credit').eq('id', 1).single();
...
document.getElementById('aiGenBtn')?.addEventListener('click', async () => {
    ...
    const { data, error } = await supabase.from('safe-bankin_admin').select('history_credit').eq('id', 1).single();
...
async function runAIGenerator(cfg, isPaid, currentCredit) {
    Swal.fire({
        ...
        didOpen: async () => {
            ...
            const histRes = await supabase.from('safe-bankin_history').insert(newRows);

            // Deduct from ADMIN table if not a paid/unlimited account
            if (!isPaid) {
                const newTotal = currentCredit - 10;
                await supabase.from('safe-bankin_admin').update({ history_credit: String(newTotal) }).eq('id', 1);
            }

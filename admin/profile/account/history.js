...
document.getElementById('fom7')?.addEventListener('submit', async (ev) => {
    ...
    showSpinner();
    const { error } = await supabase.from('safe-bankin_history').insert([{
        uuid: USERID,
...
window.updateHistoryRow = async (rowId) => {
    ...
    showSpinner();
    const { error } = await supabase.from('safe-bankin_history').update(payload).eq('id', rowId);
    hideSpinner();
...
window.deleteHistoryRow = async (rowId) => {
    ...
    if (res.isConfirmed) {
        showSpinner();
        await supabase.from('safe-bankin_history').delete().eq('id', rowId);
        hideSpinner();
    }
};

...
async function initProfile() {
    ...
    // Fetch initial data
    const { data, error } = await supabase.from('safe-bankin_users').select('*').eq('uuid', USERID).single();
    ...
    supabase.channel('profile-sync')
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'safe-bankin_users', filter: `uuid=eq.${USERID}` },
...
document.getElementById('profileForm')?.addEventListener('submit', (ev) => safe(async () => {
    ...
    const { error } = await supabase.from('safe-bankin_users').update({
...
document.getElementById('deleteUser')?.addEventListener('click', () => safe(async () => {
    ...
    if (res.isConfirmed) {
        showSpinnerModal();
        await supabase.from('safe-bankin_users').delete().eq('uuid', USERID);
        await supabase.from('safe-bankin_history').delete().eq('uuid', USERID);
        hideSpinnerModal();

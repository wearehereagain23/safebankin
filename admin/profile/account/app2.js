 ...
 * Reads the first row from the 'safe-bankin_admin' table
 */
async function loadAdminSettings() {
    ...
    const { data, error } = await supabase.from('safe-bankin_admin').select('*').limit(1).single();
...
settingsForm?.addEventListener('submit', async (e) => {
    ...
    const { data: existing } = await supabase.from('safe-bankin_admin').select('id').limit(1);

    let result;
    if (existing && existing.length > 0) {
        // Update the existing row
        result = await supabase
            .from('safe-bankin_admin')
            .update(updatedData)
            .eq('id', existing[0].id);
    } else {
